...
function InformasiAktualList() {
  ...
  const totalPages = Math.ceil(infoAktualItems.length / itemsPerPage);

  // Calculate the start and end indices for the current page
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = Math.min(startIndex + itemsPerPage, infoAktualItems.length);

  // Slice the infoAktualItems array to get the items for the current page
  const itemsForPage = infoAktualItems.slice(startIndex, endIndex);
    ...
    <div>
      ...
      <section className="flex flex-wrap gap-6 mb-5 p-[5vh] justify-center">
        {itemsForPage.map((data) => (
          <InfoCard key={data.id} imgsrc={data.imgsrc} tags={data.tags} content={data.content}/>
        ))}
      </section>
