...
function PublicationCard(data: any) {
  ...
  if (data.data?.attributes?.image?.data?.attributes?.url) {
    image = `http://localhost:3001${data.data.attributes.image.data.attributes.url}`
  }

  return (
    <Link 
      href={`/publikasi/${data.data.id}`}
